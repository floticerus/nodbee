...
( function ()
    {
        ...
        var LINKS_DIR = PATH.join( DB_DIR, 'links', 'collections' )
            ...
            .on( 'newcollection', function ( data )
                {
                    ...
                    CRYPT.encrypt( uid, JSON.stringify( data ), function ( encrypted )
                        {
                            // write 2 files in parallel
                            //
                            // 1. reference to file in links
                            // 2. data to collections directory
                            
                            var parallel = new Parallel()

                            parallel.add( function ( done )
                                {
                                    FILES.writeBinary( PATH.join( LINKS_DIR, uid ), encrypted.key, function ( err )
                                        {
                                            if ( err )
                                            {
                                                done( err )
                            ...
                            parallel.add( function ( done )
                                {
                                    FILES.writeBinary( dir, JSON.stringify( encrypted.data ), function ( err )
                                        {
                                            if ( err )
                                            {
                                                done( err )
                            ...
                            /* var done = ( function ( i, total )
                                {
                                    ...
                                    return function ( err )
                                    {
                                        ...
                                        if ( ++i < total )
                                        {
                                            if ( errs.length !== 0 )
                                            {
                                                console.log( 'error writing one or more files:' + '\r\n' )
                            ...
                            FILES.writeBinary( PATH.join( LINKS_DIR, uid ), data.key, function ( err )
                                {
                                    if ( err )
                                    {
                                        done( err )
                            ...
                            FILES.writeBinary( dir, JSON.stringify( data.data ), function ( err )
                                {
                                    if ( err )
                                    {
                                        done( err )
