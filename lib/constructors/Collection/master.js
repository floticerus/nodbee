...
( function ()
    {
        ...
        var KEYS_DIR = PATH.join( DB_DIR, 'keys', 'collections' )
            ...
            .on( 'newcollection', function ( data )
                {
                    ...
                    var uid = UID_COLLECTIONS.add( 32 ),
                        ...
                        keyFile = PATH.join( KEYS_DIR, CRYPT.makeHash( uid ) )

                    // make damn sure the id isn't taken
                    while ( FS.existsSync( collectionFile ) || FS.existsSync( keyFile ) )
                    {
                    ...
                    CRYPT.encrypt( uid, JSON.stringify( data ), function ( encrypted )
                        {
                            ...
                            parallel.add( function ( done )
                                {
                                    FILES.writeBinary( keyFile, encrypted.key, function ( err )
                                        {
                                            if ( err )
                                            {
                                                done( err )
                            ...
                            parallel.add( function ( done )
                                {
                                    FILES.writeBinary( collectionFile, JSON.stringify( encrypted.data ), function ( err )
                                        {
                                            if ( err )
                                            {
                                                done( err )
