...
( function ()
    {
        ...
        var CRYPT_LISTENERS = {
            ...
            'encryptFile': function ( uid, key, path )
            {
                ( function ( that )
                    {
                        FS.exists( path, function ( exists )
                            {
                                if ( !exists )
                                {
                                    // file does not exist, return something
                                    return that.process( 'encryptFile', uid,
                                        {
                                            key: uid,
                                ...
                                var keyBuf = new Buffer( key + process.env.NBKEY, 'hex' ),
                                    ...
                                    outPath = PATH.join( PATH.dirname( path ), uid ),

                                    inp = FS.createReadStream( path, { 'mode': 0600 } ),

                                    out = FS.createWriteStream( outPath, { 'encoding': 'binary', 'mode': 0600 } )
        ...
        process.on( 'message', function ( message, handler )
            {
                ...
                if ( !message.fn || !CRYPT_LISTENERS[ message.fn ] )
                {
                    return
                }

                CRYPT_LISTENERS[ message.fn ].call( CRYPT_PROCESS, message.uid, message.key, message.data )
            }
        )
    }
)()
