                    ...
                    FS.exists( paths.key, function ( exists )
                        {
                            ...
                            var child = SPAWN( 'openssl', [ 'genrsa', '-out', paths.key, opts.keyLength ], { stdio: 'inherit' } )
                            ...
                            FS.exists( paths.cert, function ( exists )
                                {
                                    ...
                                    var child = SPAWN( 'openssl', [ 'req', '-new', '-key', paths.key, '-out', paths.csr, '-subj', subjString ], { stdio: 'inherit' } )
                    ...
                    FS.exists( paths.cert, function ( exists )
                        {
                            ...
                            var child = SPAWN( 'openssl', [ 'x509', '-req', '-in', paths.csr, '-signkey', paths.key, '-out', paths.cert ], { stdio: 'inherit' } )
